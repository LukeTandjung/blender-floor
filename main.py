...
def import_image_and_fbx(image_path: str, fbx_path: str) -> None:
  ...
  world_corners = list(
    chain.from_iterable(
      map(lambda object: 
        map(lambda corner: 
          object.matrix_world @ corner, object.bound_box
        ), filter(
          lambda object: object.type == 'MESH', bpy.context.selected_objects
        )
      )
    )
  )
  ...
  texture_node = material.node_tree.nodes.new(type = 'ShaderNodeTextImage')
  texture_node.image = image
  material.node_tree.nodes['Principled BSDF'].inputs['Base Color'] = texture_node.outputs['Color']
  ...
  bpy.ops.mesh.primitive_plane_add(size=1.0, enter_editimode=False, align='WORLD')
